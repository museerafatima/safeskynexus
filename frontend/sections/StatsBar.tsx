  ...
  return (
    <section className="max-w-container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 md:py-12">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-4 sm:gap-x-6 md:gap-x-8 text-center">
        {stats.map((stat, i) => (
          <Reveal key={stat.label} delay={i * 100} className="px-2 group cursor-default">
            <p className="text-body text-xs sm:text-sm mb-1">{stat.label}</p>
            <p className="text-orange text-2xl sm:text-3xl md:text-4xl font-bold mb-1 break-words transition-transform duration-300 group-hover:scale-110 inline-block">
              {stat.value}
            </p>
